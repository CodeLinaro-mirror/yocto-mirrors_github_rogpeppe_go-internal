...
func (p *fakePlugin) Info() PluginInfo {
	return PluginInfo{
		...
		Cmds:        map[string]CmdInfo{"hello": {NeedStdout: true}},
	}
}
...
func (p *fakePlugin) NewTestInstance(tp TestParams) (TestInstance, error) {
	...
	return &fakeInstance{work: tp.Environ["WORK"]}, nil
}
...
func (i *fakeInstance) RunCmd(p CmdParams) (CmdResult, error) {
	...
	return CmdResult{}, fmt.Errorf("unknown command %q", p.CmdName)
}
...
func TestRoundTrip(t *testing.T) {
	...
	if cmds := c.Info().Cmds; len(cmds) != 1 || !cmds["hello"].NeedStdout {
		t.Fatalf("unexpected Info().Cmds: %v", cmds)
	}

	inst, err := c.NewTestInstance(TestParams{
		TestName: "t1",
		Environ:  map[string]string{"WORK": "/tmp/work"},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	res, err := inst.RunCmd(CmdParams{CmdName: "hello", Args: []string{"world"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	res, err = inst.RunCmd(CmdParams{CmdName: "fail"})
	if err != nil {
	...
	if _, err := inst.RunCmd(CmdParams{CmdName: "boom"}); err == nil || !strings.Contains(err.Error(), "plugin transport boom") {
	...
	for i := range n {
		...
		go func() {
			...
			inst, err := c.NewTestInstance(TestParams{
				...
				Environ:  map[string]string{"WORK": fmt.Sprintf("/w%d", i)},
			})
			if err != nil {
			...
			defer inst.Close()
			res, err := inst.RunCmd(CmdParams{CmdName: "hello", Args: []string{fmt.Sprintf("%d", i)}})
			if err != nil {
