...
// input (requests) and standard output (responses); its standard error is
// inherited from the host and may be used freely for logging. A plugin
// binary's main is essentially:
//
//	func main() {
//		if err := plugin.Main(myPlugin); err != nil {
//			log.Fatal(err)
//		}
//	}
//
...
type TestParams struct {
	...
	// Environ holds the values of the variables named in
...
type CmdInfo struct {
	...
	// NeedStderr and NeedStdout indicate whether the command produces
	// standard error or standard output respectively.
	NeedStderr bool
	NeedStdout bool
}
...
type CmdParams struct {
	// CmdName is the name of the command being run.
	CmdName string

	// Args holds the command's arguments (not including the command name).
	Args []string

	// CWD is the absolute path of the current working directory the
	// command should run in.
	CWD string

	// Environ holds the values of the variables named in the command's
...
type CmdResult struct {
	...
	// Environ holds environment variables that the command wants to set in
