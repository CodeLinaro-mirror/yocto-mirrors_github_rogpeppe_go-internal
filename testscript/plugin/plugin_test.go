...
func (demoPlugin) Info() plugin.PluginInfo {
	return plugin.PluginInfo{
		...
		Cmds: map[string]plugin.CmdInfo{
			"echo":     {NeedStdout: true},
			"warn":     {NeedStderr: true},
			"cwd":      {NeedStdout: true},
			"settoken": {},
			"fail":     {NeedStderr: true},
		},
	}
}
...
func (inst *demoInstance) RunCmd(p plugin.CmdParams) (plugin.CmdResult, error) {
	...
	case "cwd":
		return plugin.CmdResult{Stdout: []byte(p.CWD + "\n")}, nil
	case "settoken":
		if len(p.Args) != 1 {
			return plugin.CmdResult{}, fmt.Errorf("usage: settoken <value>")
		}
		return plugin.CmdResult{Environ: map[string]string{"DEMO_TOKEN": p.Args[0]}}, nil
	case "fail":
